"""End-to-end test for the LOCAL storage backend + anonymous uploads.

Drives the full guest flow (init → PUT bytes to the backend blob endpoint →
finalize with owner_token → serve → "seen?" status), plus size-cap rejection,
auto-expiry (lazy 410 + sweeper), and the authenticated path on local disk.

Bytes are written to a throwaway temp dir; the local Postgres is used for the
rows. Exits non-zero on any failed assertion.
...
Run:
    python scripts/e2e_files_local_test.py
"""
...
async def _ensure_test_user() -> str:
    async with async_session_maker() as session:
        result = await session.execute(select(User).where(User.email == "e2e-local@test.local"))
        u = result.scalar_one_or_none()
        if u is None:
            u = User(id=uuid4(), email="e2e-local@test.local", clerk_id=f"clerk_local_{uuid4().hex[:8]}")
            session.add(u)
...
def _make_token() -> TokenData:
    return TokenData(user_id=_TEST_USER_ID, email="e2e-local@test.local")
...
async def run() -> int:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://platform.test") as client:
        ...
        check("upload url is the blob endpoint",
              init["presigned_put_url"].startswith("/api/v1/files/") and "blob?token=" in init["presigned_put_url"],
              init["presigned_put_url"])
        check("owner_token returned", bool(init.get("owner_token")))
        check("expires_at set for guest", bool(init.get("expires_at")))
        check("serve_mode = stream", init["serve_mode"] == "stream")

        # === T2: PUT bytes to the backend blob endpoint ===
        print("\n[T2] PUT bytes to local blob endpoint")
        ...
        check("bytes landed on disk", any(p.endswith("report.pdf") for p in _disk_files()), str(_disk_files()))

        # === T3: blob PUT with a bad token is rejected ===
        ...
        final = r.json()
        check("status = active", final.get("status") == "active")
        check("size matches", final.get("size_bytes") == len(pdf))
        check("expires_at present on response", bool(final.get("expires_at")))

        # === T5: finalize without the token is unauthorized ===
        print("\n[T5] finalize/status without token → unauthorized")
        r2 = await client.get(f"/api/v1/files/{init['file_id']}/status")
        check("status without token → 403", r2.status_code == 403, f"got {r2.status_code}")

        # === T6: serve streams bytes + records a view ===
        print("\n[T6] serve via /f/{code} (streamed from disk)")
        ...
        check("Content-Type = application/pdf", (r.headers.get("content-type") or "").startswith("application/pdf"))

        # === T7: "seen?" status reflects the view ===
        print("\n[T7] status shows seen=true after a view")
        ...
        # === T8: declared-size cap is enforced while streaming ===
        print("\n[T8] over-size blob PUT → 413")
        r = await client.post("/api/v1/files", json={
        ...
        # === T9: lazy expiry → 410 + cleanup ===
        print("\n[T9] expired file serves 410 and is reclaimed")
        async with async_session_maker() as session:
        ...
        # === T10: sweeper reclaims expired guest files ===
        print("\n[T10] background sweeper reclaims expired files")
        gpdf = b"%PDF-1.4 sweep-me %%EOF"
        r = await client.post("/api/v1/files", json={
        ...
        async with async_session_maker() as session:
            fa = (await session.execute(select(FileAsset).where(FileAsset.id == UUID(s_init["file_id"])))).scalar_one()
            check("swept row marked deleted", fa.status == "deleted", f"status={fa.status}")

        # === T11: authenticated upload on local backend (no expiry) ===
        print("\n[T11] authenticated upload on local backend")
        app.dependency_overrides[get_current_user] = _make_token
        app.dependency_overrides[require_auth] = _make_token
        try:
            apdf = b"%PDF-1.4 authed-local %%EOF"
            r = await client.post("/api/v1/files", json={
            ...
            check("authed init: no owner_token", a_init.get("owner_token") is None)
            await client.put(a_init["presigned_put_url"], content=apdf, headers=a_init["headers"])
            r = await client.post(f"/api/v1/files/{a_init['file_id']}/finalize")  # no token needed
    ...
    if _failures:
        ...
        for f in _failures:
            print(f"  - {f}")
        return 1
    print("\033[32mAll local-backend + anonymous checks passed.\033[0m")
    return 0
